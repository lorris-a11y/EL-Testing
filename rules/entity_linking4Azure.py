...
import requests
# Import spaCy model for better possessive detection
import spacy
from flair.data import Sentence

from rules.descriptionProcessor import MultiNEREntityProcessor, get_entity_description_azure
...
from rules.spacySimple import should_skip_entity_replacement_azure
...
def get_entities(sentence_text, tagger):
    """Use Azure NER model to identify entities in the sentence, returning entity tags and positions."""
...
def get_azure_fallback_description(entity_type: str) -> str:
    """Get a generic description based on Azure entity type."""
    return {
        # Person types
        "Person": "a person",
        "PersonType": "a type of person",
...
def select_pronoun(entity_type: str, context_after: str = "", sentence: str = "") -> str:
    """Select appropriate pronoun based on Azure entity type and context."""
    print(f"\nDEBUG - Selecting pronoun:")
    print(f"Entity type: {entity_type}")
    print(f"Context after: '{context_after}'")

    # Base pronouns for Azure entity types
    base_pronouns = {
        "Person": "he",
        "PersonType": "they",  #
...
def should_skip_entity_replacement(sentence: str, entity: tuple, entity_type: str) -> bool:
    return should_skip_entity_replacement_azure(sentence, entity, entity_type)
...
def get_azure_fallback_description(entity_type: str) -> str:
    """Get fallback description for Azure entity types."""
    processor = MultiNEREntityProcessor()
    return processor._get_fallback_description(entity_type, "azure")
...
def get_entity_description(entity_text: str, entity_type: str, session=None) -> str:
    ...
    #
    if entity_type in {"DateTime", "PhoneNumber", "Email", "URL", "IP", "Quantity", "Address", "Skill"}:
        return get_azure_fallback_description(entity_type)

    return get_entity_description_azure(entity_text, entity_type, session)
    ...
    try:
        ...
        for entity_type, entity_list in entities.items():
            ...
            try:
                ...
                if not entity_intro:
                    entity_intro = f"{entity_name} is {get_azure_fallback_description(entity_type)}."
