...
from AwsUtils.aws_ner_utils import setup_aws_mock, initialize_aws_tagger, ensure_output_dir, disable_all_proxies, restore_proxy_settings

# Setup AWS mock for Flair
setup_aws_mock()

# Import the AWS entity linking method
from rules.entity_linking4Aws import get_entities, mutate_and_verify_with_knowledge_graph

#
timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
log_filename = f'TR-AWS/testRule3-{timestamp}.log'
...
def process_mutations_rule_three_aws():
    """Process mutations using Rule Three with AWS NER."""
    #
    start_time = time.time()
    logger.info(f"Starting process_mutations_rule_three_aws (Log file: {log_filename})")
    ...
    try:
        ...
        mutated_results_file = "TR-AWS/mutated_results3_test.json"
        suspicious_sentences_file = "TR-AWS/suspicious_sentences3_test.json"
        ...
        logger.critical(f"Critical error in process_mutations_rule_three_aws: {str(e)}")
...
if __name__ == "__main__":
    try:
        logger.info(f"=== Starting testRule3Aws.py (Log: {log_filename}) ===")
        process_mutations_rule_three_aws()
        logger.info("=== Completed testRule3Aws.py ===")
    except Exception as e:
