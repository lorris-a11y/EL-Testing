...
def process_mutations_rule_one():
    ...
    try:
        with open(INPUT_FILE, 'r', encoding='utf-8') as f:
            # with open('findParallel/parallel_sentences_bbcNews.json', 'r', encoding='utf-8') as f:
            data = json.load(f)
    ...
    for i, item in enumerate(data):
        ...
        for result in suspicious_results_1:
            processed_result = {
                "original_sentence": original_text,
                "mutated_sentence": result["mutated_sentence"],
                "reasons": result.get("reasons", []),  # Include reasons if available
                "original_entities": {
                    tag: [{"text": e["text"], "start": e["start"], "end": e["end"]}
                          for e in entities]
    ...
    mutated_results_filename = f'{OUTPUT_DIR}/mutated_results1.json'
    suspicious_results_filename = f'{OUTPUT_DIR}/suspicious_sentences1.json'
...
if __name__ == "__main__":
    try:
        logger.info(f"=== Starting testRule1.py (Log: {log_filename}) ===")
        ...
        logger.info("=== Completed testRule1.py ===")
    except Exception as e:
