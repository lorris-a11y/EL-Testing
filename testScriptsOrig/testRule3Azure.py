...
from AzureUtils.azure_ner_utils import setup_azure_mock, initialize_azure_tagger, ensure_output_dir, disable_all_proxies, restore_proxy_settings

# Setup Azure mock for Flair
setup_azure_mock()

# Import the modified entity linking method
from rules.entity_linking4Azure import get_entities, mutate_and_verify_with_knowledge_graph

#
timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
log_filename = f'TR-Azure/testRule3-{timestamp}.log'
...
def process_mutations_rule_three_azure():
    """Process mutations using Rule Three with Azure NER."""
    #
    start_time = time.time()
    logger.info(f"Starting process_mutations_rule_three_azure (Log file: {log_filename})")
    ...
    try:
        ...
        # Create a session specifically for DBpedia requests with proxy
        dbpedia_session = requests.Session()
        dbpedia_session.proxies = {
            "http": "your_proxy_here",
            "https": "your_proxy_here",
        }
        logger.info("DBpedia session created with proxy configuration.")

        # Read input data
        logger.info("Reading data from JSON file")
        try:
            with open('txt2json/bbcNews2.json', 'r', encoding='utf-8') as f:
                data = json.load(f)
        ...
        mutated_results_file = "your_mutated_file_path"
        suspicious_sentences_file = "your_suspicious_file_path"
        ...
        logger.critical(f"Critical error in process_mutations_rule_three_azure: {str(e)}")
...
if __name__ == "__main__":
    try:
        logger.info(f"=== Starting testRule3Azure.py (Log: {log_filename}) ===")
        process_mutations_rule_three_azure()
        logger.info("=== Completed testRule3Azure.py ===")
    except Exception as e:
